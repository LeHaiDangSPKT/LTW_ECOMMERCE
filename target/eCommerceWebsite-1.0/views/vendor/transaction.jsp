      ...
      <div class="dataTables_paginate paging_simple_numbers">
        <ul class="pagination">
          <li class="paginate_button page-item ${tag == 1 ? "disabled" : ""}">
            <a
                    href="${pageContext.request.contextPath}/vendor/transaction?index=${tag - 1}"
                    class="page-link">Previous
            </a>
          </li>
          <c:forEach begin="1" end="${endP}" var="i">
            <li class="paginate_button page-item ${i == tag ? "active" : ""}">
              <a href="${pageContext.request.contextPath}/vendor/transaction?index=${i}"
                 class="page-link">${i}</a>
            </li>
          </c:forEach>
      ...
      <div class="modal-content">
        <form action="<c:url value="/vendor/transaction"/>" method="post">
          ...
          <div class="modal-body">
            <div class="mb-3" style="background-color: #f0f5f4; border-radius: 5px; padding: 5px 10px; line-height: 39px">
              <i class="fa-solid fa-book text-primary"></i>
              <label style="margin: 0">Số dư Ví:</label>
              <span class="font-weight-bold; ml-3" style="display: inline-block">${wallet} vnđ</span>
            </div>
            <div>
              <label>Số tiền cần rút:</label>
              <input class="form-control font-weight-bold font-size-18" type="text" placeholder="0đ" name="amount">
            </div>
            <div class="mt-3">
              <span>Rút về Ví của: </span>
...
<script>
  ...
  function Search() {
    ...
    if (CheckDate()) {
      window.location.href = "${UrlTransaction}" + "?start=" + dateStart + "&end=" + dateEnd;
    } else {
